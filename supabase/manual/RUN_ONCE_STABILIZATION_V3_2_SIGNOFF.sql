-- CHANANYA CLINICAL OS v3.2 — SIGNOFF / LOCK / AUDIT INSTALLER
-- Run once in Supabase SQL Editor before merging PR v3.2.
...
create policy clinical_record_signoffs_read on public.clinical_record_signoffs for select to authenticated using (public.has_role(array['super_admin','admin','practitioner','doctor','pharmacy']));
drop policy if exists clinical_record_signoffs_write on public.clinical_record_signoffs;
create policy clinical_record_signoffs_write on public.clinical_record_signoffs for all to authenticated using (public.has_role(array['super_admin','admin','practitioner','doctor'])) with check (public.has_role(array['super_admin','admin','practitioner','doctor']));
...
create index if not exists idx_clinical_audit_encounter on public.clinical_record_audit_events(encounter_id,created_at desc);
...
create policy clinical_record_audit_events_read on public.clinical_record_audit_events for select to authenticated using (public.has_role(array['super_admin','admin','practitioner','doctor']));
grant select on public.clinical_record_audit_events to authenticated;

create or replace function public.sign_clinical_record_complete(p_encounter_id uuid,p_signer_name text default null,p_license_no text default null,p_reason text default 'Complete clinical record sign-off')
...
begin
  ...
  if not exists(select 1 from public.clinical_treatment_plans where encounter_id=p_encounter_id) and not exists(select 1 from public.clinical_treatment_sessions where encounter_id=p_encounter_id) then raise exception 'TREATMENT_REQUIRED_BEFORE_SIGNOFF'; end if;
  ...
  on conflict(encounter_id,record_section) do update set signer_id=auth.uid(),signer_name=excluded.signer_name,professional_license_no=excluded.professional_license_no,signed_at=now(),lock_record=true,reason=excluded.reason returning * into v_row;
...
begin
  ...
  update public.clinical_record_signoffs set lock_record=false,reason='Unlocked for amendment: '||btrim(p_reason) where encounter_id=p_encounter_id and record_section='complete_record';
  if not found then raise exception 'SIGNED_RECORD_NOT_FOUND'; end if;
  insert into public.clinical_record_audit_events(encounter_id,event_type,record_section,actor_id,reason) values(p_encounter_id,'UNLOCK_FOR_AMENDMENT','complete_record',auth.uid(),btrim(p_reason));
  return true;
end;
$$;
...
begin
  ...
  if exists(select 1 from public.clinical_record_signoffs where encounter_id=v_encounter and record_section='complete_record' and lock_record=true) then raise exception 'CLINICAL_RECORD_LOCKED'; end if;
...
with checks(name,ready) as (
 values
 ('clinical_record_signoffs',to_regclass('public.clinical_record_signoffs') is not null),
 ('clinical_record_audit_events',to_regclass('public.clinical_record_audit_events') is not null),
 ('sign_clinical_record_complete',to_regprocedure('public.sign_clinical_record_complete(uuid,text,text,text)') is not null),
 ('unlock_clinical_record_for_amendment',to_regprocedure('public.unlock_clinical_record_for_amendment(uuid,text)') is not null),
 ('prevent_locked_clinical_record_mutation',to_regprocedure('public.prevent_locked_clinical_record_mutation()') is not null)
), s as (select count(*) total,count(*) filter(where ready) ok from checks)
select 'CHECK' as object_type,name,case when ready then 'READY' else 'MISSING' end status from checks
union all
select 'SUMMARY','CHANANYA_CLINICAL_OS_V3_2',case when ok=total then 'READY '||ok||'/'||total else 'MISSING '||(total-ok)||' OF '||total end from s
