...
-- Run once in Supabase SQL Editor after the existing Clinical v3, TTM knowledge and OPD foundation migrations.

begin;

-- Body Pain Map structured fields
...
alter table public.body_pain_points add constraint body_pain_points_side_check check (side is null or side in ('left','right','bilateral','midline','not_specified'));
...
-- Transaction-safe Treatment Session
...
-- Atomic Structured TTM Diagnosis + Samutthan Context
...
with required_objects(object_type,object_name,is_ready) as (
  ...
  select count(*) total_checks,count(*) filter(where is_ready) ready_checks,count(*) filter(where not is_ready) missing_checks from required_objects
)
