...
-- Appointment operators must be authorized by an active membership in the
-- current clinic. The single-clinic owner is a real clinic operator even when
-- the profile also carries the platform-level super_admin marker.
  ...
  select public.current_clinic_id() is not null
    and exists (
      ...
      where m.clinic_id = public.current_clinic_id()
        ...
        and m.active
        and (
          m.clinic_role in ('owner','admin','reception')
          or p.system_role in ('admin','super_admin')
        )
    );
$$;

revoke all on function public.is_appointment_operator() from public, anon, authenticated, service_role;
grant execute on function public.is_appointment_operator() to authenticated;
