...
begin
  ...
  insert into public.clinical_record_signoffs(
    encounter_id,record_section,signer_id,signer_name,professional_license_no,signed_at,lock_record,reason
  ) values(
    p_encounter_id,'complete_record',auth.uid(),nullif(btrim(p_signer_name),''),nullif(btrim(p_license_no),''),now(),true,p_reason
  )
  ...
  values(p_encounter_id,'SIGN_AND_LOCK','complete_record',auth.uid(),p_reason,
    jsonb_build_object('license_no',p_license_no,'signed_at',now()));
...
begin
  ...
  if exists(
    select 1 from public.clinical_record_signoffs
     where encounter_id=v_encounter and record_section='complete_record' and lock_record=true
  ) then
...
declare t text;begin
  foreach t in array array[
    'clinical_examination_findings','ttm_structured_diagnoses','ttm_diagnostic_contexts',
    'clinical_treatment_plans','body_pain_points','ttm_opd_histories',
    'clinical_treatment_sessions','clinical_followup_notes'
  ] loop
