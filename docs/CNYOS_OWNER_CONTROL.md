...
Supabase can link Google and GitHub identities with the same confirmed email. A linked account is accepted only after the server verifies its Supabase session, OAuth AMR, confirmed Owner allowlist and an additional Google credential. The browser forwards the current SDK session's `provider_token` in `X-Owner-Google-Token` to the same-origin Owner endpoints. The server calls the fixed Google OpenID Connect UserInfo endpoint with that credential, prohibits redirects, and requires a verified email matching the Owner and a `sub` matching the trusted Supabase Google identity. GitHub OAuth alone, editable user metadata and matching email without the Google subject are insufficient. Existing Google-only accounts retain their previous authorization path.

The app does not create another browser token store, log credentials or send the Supabase service key to Google. Supabase does not refresh Google provider tokens: when the proof is missing or expired, use **ออกจากระบบแล้วเข้า Google ใหม่** in Owner recovery to obtain a fresh Google session. Do not unlink another identity or change account roles to work around this check. References: [Supabase identity linking](https://supabase.com/docs/guides/auth/auth-identity-linking), [Google login provider tokens](https://supabase.com/docs/guides/auth/social-login/auth-google), [Google UserInfo](https://developers.google.com/identity/openid-connect/reference).
...
The browser sends folder URLs/IDs and Owner authentication credentials only. `/api/owner-drive` fixes the environment from `BACKUP_ENVIRONMENT`, authenticates the allowlisted Google Owner, resolves the encrypted Google Drive credential from a site-scoped, strong-consistency Netlify Blob, obtains the Drive access token server-side, verifies an OAuth user's exact Google email when that adapter is selected, and verifies that the credential can add children to all five distinct folders. Every folder must be an untrashed direct child of `GOOGLE_DRIVE_EXPECTED_ROOT_FOLDER_ID` before the assignment can commit. The browser never receives the credential document, refresh token, Blob wrap key, `SUPABASE_SERVICE_ROLE_KEY`, `BACKUP_ENCRYPTION_KEY_BASE64` or Google Drive access token. Provisioning and rotation are defined in [GOOGLE_SERVICE_ACCOUNT_BLOB.md](./GOOGLE_SERVICE_ACCOUNT_BLOB.md).
