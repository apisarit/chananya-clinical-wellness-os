...
Super Admin workflow: user signs in with Google → open **ศูนย์ควบคุม → ผู้ใช้และสิทธิ์ → โหลดรายชื่อใหม่** → choose the pending user under **กำหนด Role ปฏิบัติงาน** → choose a department and save. `viewer` permits U Synthesize without clinical privileges. Membership assignment continues through the existing audited `admin_assign_staff_role` RPC. System-role promotion is offered only after the account is an active clinic member.
