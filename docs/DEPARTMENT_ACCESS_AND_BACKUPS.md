...
- Browser `INSERT`, `UPDATE` and `DELETE` grants are revoked for protected product and pharmacy tables.
- Composite tenant foreign keys prevent a sale item, allocation, lot or movement from referencing another clinic.
...
5. Apply `202608270500_department_persistence_and_drive_export.sql` before deploying the worker.
